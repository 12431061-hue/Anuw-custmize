...
const geometryScale = { shoulder: 0.323, body: 0.255, sleeveY: 0.32 * 0.22, hemY: (671.99 - 196.94) / 700 };
...
function getGeometry() {
  const shoulderOffset = (state.shoulder - 460) * 0.323;
  const sleeveOffset = (state.sleeve - 220) * 0.32;
  const bodyOffset = (state.body - 520) * 0.255;
  const lengthRatio = state.length / 700;
  const xLeftShoulder = 142.56 - shoulderOffset;
  const xRightShoulder = 440.02 + shoulderOffset;
  const xLeftBody = 158.7 - bodyOffset;
  const xRightBody = 423.88 + bodyOffset;
  ...
  return { shoulderOffset, xLeftShoulder, xRightShoulder, xLeftBody, xRightBody, xLeftHem, xRightHem, yBody, yHem, leftSleeveTop, rightSleeveTop, leftCuff, rightCuff, hasSleeve };
}
...
function setPathGeometry(g) {
  const isSourceSize = state.shoulder === 460 && state.sleeve === 220 && state.body === 520 && state.length === 700;
  // 襟、二重線、ステッチは支給SVGの形状を保つ。寸法変化の対象にしない。
  paths.neckLeft.setAttribute('d', 'M236.74,196.94s1.89,74.36,54.64,73.96');
  paths.neckRight.setAttribute('d', 'M346.03,196.94s-1.89,74.36-54.64,73.96');
  paths.neckInnerLeft.setAttribute('d', 'M227.06,199.47s2.23,82.33,64.65,81.89');
  paths.neckInnerRight.setAttribute('d', 'M356.37,199.47s-2.23,82.33-64.65,81.89');
  paths.stitchTop.setAttribute('d', 'M238,209.48s.19.06.57.18M239.51,209.94c8.48,2.49,51.56,13.64,103.35-.18M343.36,209.63c.19-.05.39-.1.58-.16');
  paths.stitchBottom.setAttribute('d', 'M238.74,214.7s.19.06.57.18M240.26,215.16c8.48,2.49,51.56,13.64,103.35-.18M344.1,214.85c.19-.05.39-.1.58-.16');
  if (isSourceSize) {
    pathEl.setAttribute('d', sourcePath);
    clipPathEl.setAttribute('d', sourcePath);
    paths.sleeveSeamLeft.setAttribute('d', 'M142.56,219.56s4.07,110.78-10.97,136.41');
    paths.sleeveSeamRight.setAttribute('d', 'M440.45,219.56s-4.07,110.78,10.97,136.41');
    return;
  }

  const outerPath = g.hasSleeve
    ? `M 344.74 196.94 L ${g.xRightShoulder} 219.56 L ${g.rightSleeveTop.x} ${g.rightSleeveTop.y} L ${g.rightCuff.x} ${g.rightCuff.y} L ${g.xRightBody + 27.11} 355.98 L ${g.xRightBody} ${g.yBody} L ${g.xRightHem} ${g.yHem} H ${g.xLeftHem} L ${g.xLeftBody} ${g.yBody} L ${g.xLeftBody - 27.11} 355.98 L ${g.leftCuff.x} ${g.leftCuff.y} L ${g.leftSleeveTop.x} ${g.leftSleeveTop.y} L ${g.xLeftShoulder} 219.56 L 237.84 196.94 Q 291.41 214.75 344.74 196.94 Z`
    : `M 344.74 196.94 L ${g.xRightShoulder} 219.56 L ${g.xRightBody} ${g.yBody} L ${g.xRightHem} ${g.yHem} H ${g.xLeftHem} L ${g.xLeftBody} ${g.yBody} L ${g.xLeftShoulder} 219.56 L 237.84 196.94 Q 291.41 214.75 344.74 196.94 Z`;
...
function setDimension(key, value) {
  ...
  if (state.linked && (key === 'body' || key === 'shoulder')) {
    const pairedKey = key === 'body' ? 'shoulder' : 'body';
    state[pairedKey] = roundStep(clamp(state[pairedKey] + nextValue - previousValue, state.min[pairedKey], state.max[pairedKey]));
  }
  enforceGarmentBounds();
  updateUI();
}
